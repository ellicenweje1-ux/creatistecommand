  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-ink/55">Here's the state of the kitchen today.</p>
      </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-3">
        <div className="space-y-5 lg:col-span-2">
          <Card title="Upcoming bookings" action={<Link to="/app/bookings" className="text-xs font-medium text-copper">View all</Link>} pad={false}>
            {data.upcoming_bookings.length === 0 ? (
              <p className="p-5 text-sm text-ink/45">No bookings in the next 3 weeks. <Link className="text-copper" to="/app/bookings">Add one →</Link></p>
            ) : (
                  ...
                  <li key={b.id}>
                    <Link to={`/app/bookings/${b.id}`} className="flex items-center gap-4 px-4 py-3 transition-colors hover:bg-parchment/50">
                      <div className="w-14 shrink-0 text-center">
                        ...
                        <p className="text-[10px] uppercase tracking-wider text-ink/45">{fmtDate(b.date).split(' ')[2]}</p>
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-medium">{b.title}</p>
                        <p className="truncate text-xs text-ink/50">{b.guest_count} guests · {b.venue_name || 'venue TBC'} · {relDays(b.date)}</p>
                      </div>
          ...
          <Card title="Next tasks" action={<Link to="/app/tasks" className="text-xs font-medium text-copper">All tasks</Link>} pad={false}>
            {data.tasks.next.length === 0 ? <p className="p-5 text-sm text-ink/45">Nothing pending. Enjoy the calm.</p> : (
              <ul className="divide-y divide-line/70">
                {data.tasks.next.map((t) => (
                  <li key={t.id} className="flex items-center gap-3 px-4 py-2.5">
                    <Icon name={t.status === 'doing' ? 'clock' : 'checks'} size={15} className="shrink-0 text-ink/35" />
                    <span className="min-w-0 flex-1 truncate text-sm">{t.title}</span>
                    <span className="shrink-0 text-xs text-ink/45">{t.due_date ? fmtDate(t.due_date) : ''}</span>
          ...
          <Card title="Orders in transit" action={<Link to="/app/orders" className="text-xs font-medium text-copper">All orders</Link>} pad={false}>
            {data.orders_in_transit.length === 0 ? <p className="p-5 text-sm text-ink/45">No deliveries on the way.</p> : (
              <ul className="divide-y divide-line/70">
                {data.orders_in_transit.map((o) => (
                  <li key={o.id} className="flex items-center gap-3 px-4 py-2.5">
                    <Icon name="truck" size={15} className="shrink-0 text-ink/35" />
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm">{o.supplier} <span className="text-ink/40">— {o.items_summary}</span></p>
                    </div>
                    <span className="shrink-0 text-xs text-ink/45">{o.expected_date ? `eta ${fmtDate(o.expected_date)}` : ''}</span>
        ...
        <div className="space-y-5">
          <Card title="Use it or lose it" action={<Link to="/app/inventory" className="text-xs font-medium text-copper">Inventory</Link>} pad={false}>
            {data.expiring.length === 0 ? <p className="p-5 text-sm text-ink/45">Nothing expiring this week.</p> : (
              <ul className="divide-y divide-line/70">
                {data.expiring.map((i) => (
          ...
          <Card title="Running low" pad={false}>
            {data.low_stock.length === 0 ? <p className="p-5 text-sm text-ink/45">Stock levels look healthy.</p> : (
              <ul className="divide-y divide-line/70">
                {data.low_stock.map((i) => (
                  <li key={i.id} className="px-4 py-2.5 text-sm">
                    <div className="flex justify-between"><span className="truncate">{i.name}</span><span className="text-ink/45">{i.quantity} {i.unit}</span></div>
                  </li>
                ))}
              </ul>
            )}
          ...
          <Card title="Shopping lists" action={<Link to="/app/shopping" className="text-xs font-medium text-copper">All lists</Link>} pad={false}>
            {data.shopping_lists.length === 0 ? <p className="p-5 text-sm text-ink/45">No open lists.</p> : (
              <ul className="divide-y divide-line/70">
                {data.shopping_lists.map((sl) => (
                  <li key={sl.id} className="px-4 py-3">
                    <div className="mb-1.5 flex justify-between text-sm">
                      <span className="truncate font-medium">{sl.title}</span>
                      <span className="shrink-0 text-xs text-ink/45">{sl.purchased_items}/{sl.total_items}</span>
                    </div>
          ...
          <Card title="Pinned ideas" action={<Link to="/app/ideas" className="text-xs font-medium text-copper">Ideas</Link>} pad={false}>
            {data.pinned_ideas.length === 0 ? <p className="p-5 text-sm text-ink/45">Pin your best sparks of genius.</p> : (
              <ul className="divide-y divide-line/70">
                {data.pinned_ideas.map((i) => (
                  <li key={i.id} className="px-4 py-2.5">
                    <p className="text-sm font-medium">{i.title || 'Untitled idea'}</p>
                    <p className="mt-0.5 line-clamp-2 text-xs text-ink/50">{i.content}</p>
                  </li>
                ))}
              </ul>
            )}
