...
from ..models import (
    Booking, Client, Design, Expense, Invoice, OnlineOrder, RoutePlan, ShoppingList, Task,
)
...
def workspace(booking_id: int, db: Session = Depends(get_db), user=Depends(require_active)):
    booking = get_owned(db, Booking, booking_id, user.id)

    def linked(model, order):
        return [
            to_dict(o)
            for o in db.query(model)
            .filter(model.user_id == user.id, model.booking_id == booking_id)
            .order_by(order)
            .all()
        ]
